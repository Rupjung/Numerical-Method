...
int i, j, k;
float pivot, factor, sum; for(k=1;k<=n-1;k++){
pivot=a[k][k];
for(i=k+1;i<=n;i++){
factor-a[i][k]/pivot;
for(j=k+1;j<=n;j++)
...
scanf("%d", &mp);
printf("\n Input x and y values (one set on each line): "); for(i=1;i<=n;i++)
scanf("%f%f", &x[i], &y[i]);
/*Number of polynomial coefficients*/
m=mp+1;
/*Computation of elements of c and b*/
normal(x, y, c, b, n, m);
/*Computation of coefficients a(1) to a(m)*/ gauss(m, c, b, a);
